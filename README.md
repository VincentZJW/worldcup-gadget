...
- After committing feed changes, the workflow purges the default jsDelivr feed URL so desktop refreshes do not keep reading stale CDN data.
...
- workflow 提交 feed 变化后会清理默认 jsDelivr feed URL 的缓存，避免桌面端刷新时继续读到旧 CDN 数据。
